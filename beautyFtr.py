...
def get_spat_arrng_ftrs(gray_img):
    ...
    resized_img = transform.resize(gray_img, (600, 600), mode='constant', anti_aliasing=False)
...
def calc_color_ftrs(h, s, v):
    ...
    return dict(pleasure=pleasure, arousal=arousal, dominance=dominance,
                hsv_itten_std_h=np.std(counts_hue), hsv_itten_std_s=np.std(counts_saturation),
                hsv_itten_std_v=np.std(counts_brightness))
...
def __main__():
    ...
    parser.add_argument('-img_lst_fp', '--img_file_list_full_path',
                        help='Full path to the image file list containing images list with full path', required=False)
    parser.add_argument('-path', '--img_file_folder_path', help='Path where all the image files are located',
                        required=False)
    parser.add_argument('-img_lst', '--img_file_list',
                        help='Full path to the image file list containing images list with only filenames',
                        required=False)
...
if __name__ == "__main__":
    #__main__()
    print(extr_beauty_ftrs("C:\\Users\\chand\\Desktop\\1234.jpg"))
